...
const faqItems = [
  { question: 'Что делать с решением?', answer: 'Оставьте ссылки на код, например на GitHub, и на live-просмотр вашего решения в комментариях.' },
  { question: 'Нужна помощь в выполнении задачи?', answer: 'Смотрите решения других участников и спрашивайте совета в комментариях ниже.' },
...
<template>
  <div class="space-y-3">
    <div v-for="(item, index) in faqItems" :key="index" class="border border-gray-200 p-4 rounded-lg overflow-hidden">
      ...
      <!-- Ответ -->
      <div v-show="openIndex === index" class="pt-4 px-4 text-gray-600" v-html="item.answer"></div>
    </div>
  </div>
</template>