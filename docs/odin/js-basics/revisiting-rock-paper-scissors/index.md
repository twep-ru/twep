...
2. В нашем пользовательском интерфейсе игрок должен иметь возможность играть в игру, нажимая на кнопки, а не вводя свой ответ в prompt.
   ...
   4. Отображайте текущий счёт и объявляйте победителя игры, как только один из игроков наберет 5 очков.

   5. Вероятно, вам придется рефакторить (перерабатывать/переписывать) ваш оригинальный код, чтобы он работал для этого. Это нормально! Переработка старого кода — важная часть жизни программиста.

3. Как только вы закончите с вашим пользовательским интерфейсом и убедитесь, что всё удовлетворительно, убедитесь, что все ваши изменения закоммичены в ветку `rps-ui` с помощью `git status` перед продолжением.

4. Теперь давайте посмотрим, как мы можем объединить изменения из нашей ветки `rps-ui` обратно в нашу основную ветку.

   1. Переключитесь на ветку, В которую мы хотим объединить, т.е. `main`, с помощью команды `git checkout main`.

   2. Теперь давайте объединим нашу ветку `rps-ui` с `main`, нашей текущей веткой, с помощью `git merge rps-ui`.

   3. Если всё пройдет хорошо, наша ветка `rps-ui` теперь успешно объединена с `main`! Используйте `git log`, и вы увидите все коммиты, которые вы сделали в вашей ветке функциональности, поверх коммитов, которые вы сделали в основной ветке. Теперь наш последний шаг!

   4. Давайте запушим нашу основную ветку в наш удаленный репозиторий, выполнив `git push origin main`. Перейдите в ваш репозиторий на GitHub, и вы увидите, что наша основная ветка будет содержать все изменения и коммиты, которые вы сделали в ветке `rps-ui`. Поздравляем! Вы успешно запушили вашу первую функцию в вашу production ветку!

   5. Теперь, когда у нас есть весь код в основной ветке, нам больше не нужна наша ветка `rps-ui`. Давайте проведем очистку, как локально, так и в удаленном репозитории. Удалите ветку из нашего локального репозитория с помощью `git branch -d rps-ui`, а также удалите её из удаленного репозитория на GitHub с помощью `git push origin --delete rps-ui`. Поздравляем, мы закончили нашу очистку!

5. Опубликуйте проект на GitHub Pages и добавьте ссылку на живой просмотр в урок проекта [на The Odin Project](https://www.theodinproject.com/lessons/foundations-rock-paper-scissors).
...
Ветки в Git — это альтернативные версии вашего кода, которые позволяют работать над разными функциями или изменениями одновременно, не влияя друг на друга. Все ветки исходят от основной ветки (main) или других веток.
...
Новую ветку можно создать с помощью команды `git branch <имя_ветки>`, а затем переключиться на неё с помощью `git checkout <имя_ветки>`. Также можно создать и сразу переключиться на новую ветку одной командой: `git checkout -b <имя_ветки>`.

</details>

**Как объединить ветку обратно с main?**

<details>
<summary>Ответ</summary>

Чтобы объединить ветку с main, нужно сначала переключиться на ветку main командой `git checkout main`, а затем выполнить слияние с помощью команды `git merge <имя_ветки>`.

</details>

**Какой еще один случай использования веток?**

<details>
<summary>Ответ</summary>

Ветки можно использовать для совместного использования кода с другими разработчиками, когда вы хотите показать свой код или получить помощь, но не хотите коммитить незавершенный или сломанный код в основную ветку.
...
- Прочитайте главу [«Основы ветвления и слияния» от Git-SCM](https://git-scm.com/book/ru/v2/%d0%92%d0%b5%d1%82%d0%b2%d0%bb%d0%b5%d0%bd%d0%b8%d0%b5-%d0%b2-Git-%d0%9e%d1%81%d0%bd%d0%be%d0%b2%d1%8b-%d0%b2%d0%b5%d1%82%d0%b2%d0%bb%d0%b5%d0%bd%d0%b8%d1%8f-%d0%b8-%d1%81%d0%bb%d0%b8%d1%8f%d0%bd%d0%b8%d1%8f), чтобы визуализировать и понять, как ветвление и слияние используются в реальных рабочих процессах, и узнать, почему это такой удивительный инструмент.

- Сделайте пуш локальных коммитов в удаленные ветки проще с помощью команды `git push -u origin <ветка>`. Она автоматически связывает локальную ветку, которую вы пушите, с удаленной. Прочитайте статью [«Git push - что это такое»](https://www.cloud4y.ru/blog/what-is-git-push/) и коммитьте быстрее в удаленную ветку с помощью команды git push.

- Изучите ветвление в Git, пройдя через этот [интерактивный визуализатор от Питера Коттла](https://learngitbranching.js.org/?locale=ru_RU). Вы можете изучить основные команды о ветвлении, одновременно наблюдая, как команды, которые вы пишете, влияют на ваш визуально представленный древовидный граф веток.

- Сохраните в закладки [шпаргалку с командами Git](https://devsheet.ru/git.html).