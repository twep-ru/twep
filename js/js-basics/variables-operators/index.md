...
7. **Ознакомьтесь со следующими статьями**, чтобы углубить свои знания:
    ...
    - Эта статья на MDN по [математическим операциям в JavaScript](https://https://developer.mozilla.org/ru/docs/Web/JavaScript/Guide/Expressions_and_operators) рассматривает ту же тему с немного другой точки зрения и также рассказывает, как применять базовые математические операции в JavaScript. С числами можно делать гораздо больше, но для начала вам нужно освоить именно это.
...
Ответьте на следующие вопросы. Если не знаете ответа — кликните на него, чтобы вернуться к соответствующей части урока.
...
**Ответ:** `var` — потому что он имеет странное поведение в контексте области видимости и поднятия (hoisting).

### 3. Какие правила нужно соблюдать при именовании переменных?
**Ответ:** Имя должно начинаться с буквы, `$` или `_`. Не может быть ключевым словом (например, `let`, `if`). Лучше использовать стиль `camelCase`.

### 4. Что происходит при сложении чисел и строк?
**Ответ:** Число преобразуется в строку, и результатом будет строка. Например: `5 + "5"` → `"55"`.

### 5. Как работает оператор остатка (`%`)?
**Ответ:** Возвращает остаток от деления. Например: `10 % 3` → `1`.

### 6. В чём разница между `==` и `===`?
**Ответ:** `==` сравнивает значения с приведением типов, `===` — строгое сравнение без приведения типов. Например: `5 == "5"` → `true`, `5 === "5"` → `false`.

### 7. Когда возникает результат `NaN`?
**Ответ:** Когда операция с числами невозможна. Например: `parseInt("abc")` или `0 / 0`.

### 8. Как увеличивать и уменьшать число?
**Ответ:** С помощью операторов `++` (инкремент) и `--` (декремент). Например: `let x = 5; x++; console.log(x);` → `6`.
...
**Ответ:** Приоритет определяет порядок выполнения операций. Например, умножение выполняется раньше сложения. Вы можете использовать скобки, чтобы изменить порядок.

### 11. Как открыть инструменты разработчика и консоль?
**Ответ:** Нажмите правой кнопкой мыши на странице → «Инспектировать» → выберите вкладку **Console**.

### 12. Как выводить информацию в консоль?
**Ответ:** С помощью команды `console.log()`.

### 13. Что делает унарный плюс (`+"10"`) со строковым представлением числа?
**Ответ:** Преобразует строку в число. Например: `typeof +"10"` → `"number"`.

## Дополнительные материалы

- [MDN: What is JavaScript?](https://developer.mozilla.org/en-US/docs/Web/JavaScript/Guide/Introduction)
- [JavaScript.info: Introduction](https://javascript.info/introduction)

Если вы выполнили все задания и поняли ответы на контрольные вопросы — вы отлично справились! Теперь вы готовы двигаться дальше и изучать условия, циклы и функции.